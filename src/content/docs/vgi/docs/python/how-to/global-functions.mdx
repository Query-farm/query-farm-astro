---
title: Publish global functions
description: "Publish selected worker functions into DuckDB's global namespace so they can be called without naming the attached catalog."
---

import CodeBlock from '../../../../../../components/ui/CodeBlock.astro';

How a worker asks the client to publish selected functions into DuckDB's global function namespace,
so they're callable without naming the attached catalog. Useful for diagnostics, converters, and
format helpers that aren't *about* the catalog's data. Requires vgi-python **0.24.0+** (protocol
1.3.0).
...
<CodeBlock code={`ATTACH 'acme' AS acme (TYPE vgi, LOCATION 'acme-worker');
...
<CodeBlock code={`from vgi.catalog import Catalog, Schema
...
<table>
<thead>
<tr><th>prefix</th><th>function name</th><th>published as</th></tr>
</thead>
<tbody>
<tr><td><code>"acme"</code></td><td><code>table_info</code></td><td><code>acme_table_info</code></td></tr>
<tr><td><code>None</code></td><td><code>table_info</code></td><td><code>table_info</code></td></tr>
</tbody>
</table>
...
- **[Expose a catalog](/vgi/docs/python/how-to/catalogs/)** — where `global_functions` is declared.
- **[API Reference — catalog](/vgi/docs/python/api/vgi-catalog/)** — `Catalog`, `Schema`, and
  `CatalogAttachResult`.
