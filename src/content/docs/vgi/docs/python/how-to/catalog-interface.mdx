...
For the high-level model (how a worker becomes a catalog, how functions get qualified names), start
with [Expose a catalog](/vgi/docs/python/how-to/catalogs/). For the exhaustive class, method, and
field catalog — every signature, every attribute — see the [Catalogs API
reference](/vgi/docs/python/api/vgi-catalog/); this page links there rather than restating it.

## How catalog methods dispatch

Catalog operations are ordinary `vgi_rpc` typed methods: each has its own request/response with
...
For most cases you don't implement `CatalogInterface` directly — you declare the catalog with the
...
Back a table with a `TableFunctionGenerator` and the table's column schema is derived from the
...
When a table isn't backed by a function, supply the column schema yourself. These tables **require**
...
For tables with explicit columns, `table_scan_function_get()` tells the VGI DuckDB extension *which
...
reference](/vgi/docs/python/api/vgi-catalog/). A typical implementation points a table at Parquet on
S3:
...
The worker emits structured `_logger.info` records and Sentry breadcrumbs for catalog lifecycle
...
`ReadOnlyCatalogInterface` is a convenience base for catalogs without DDL — every modification
...
`catalog_interface` on the worker; to disable the catalog entirely, set both `catalog_interface =
None` and `catalog_name = None`.

## Transactions

Transactions are optional. Set `supports_transactions=True` on the `CatalogAttachResult` and
...
For stateful catalogs, `CatalogStorage` persists attachment and transaction state across worker
...
A table backed by a remote API often can't be scanned wholesale — the upstream needs a key. Set
...
The shape is conjunctive normal form: the outer list is an **AND** of groups, each inner group an
**OR** of dotted-path column references.

<CodeBlock code={`# "accession_number AND one of (ticker, cik)"
required_filters=[["accession_number"], ["ticker", "cik"]]`} lang="python" tryable={false} />
...
<Callout type="note" title="Renamed in 0.15.0">
...
A catalog can ask the client to `ATTACH` **other** catalogs alongside it, so multi-branch catalog
...
<CodeBlock code={`from vgi.catalog import AttachCatalogInfo
...
- **Namespace the alias.** Two workers both claiming `"lake"` on one client is a collision, and
collisions are rejected — never silently merged.
- **Choose `required` deliberately.** When false, a failure is logged and skipped, and branches
referencing the companion then error at bind instead.
...
- **The high-level model** → [Expose a catalog](/vgi/docs/python/how-to/catalogs/) — how a worker
becomes a catalog, and the `ATTACH` walkthrough.
- **Publish functions globally** → [Publish global functions](/vgi/docs/python/how-to/global-functions/).
- **Add your own COPY formats** → [Add a custom COPY format](/vgi/docs/python/how-to/copy-formats/).
- **Every class, method, and field** → [Catalogs API reference](/vgi/docs/python/api/vgi-catalog/).
- **Function execution phases** → [Function lifecycle](/vgi/docs/concepts/lifecycle/).
