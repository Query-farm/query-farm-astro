...
```sql
SELECT bucket,
       bitfilters_duckdb_bloom_filter_create(bitfilters_duckdb_hash(key)) AS bf
FROM keys
GROUP BY bucket;
...
WHERE z.bucket = e.bucket
  AND bitfilters_duckdb_bloom_filter_probe(z.bf, bitfilters_duckdb_hash(e.key));
```

The `bitfilters_duckdb_hash` is required input — it produces hashes that match DuckDB's internal Bloom filter probe layout.
