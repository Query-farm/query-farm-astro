...
<div class="flex items-center gap-3 mb-6 not-prose text-soil-600">
  ...
  <p class="text-sm">Recipes for distinct counts, quantiles, and heavy hitters with <a href="https://datasketches.apache.org/" class="text-harvest-700 underline underline-offset-2 decoration-harvest-300 hover:decoration-harvest-700">Apache DataSketches</a>.</p>
</div>
...
SELECT
  ...
  datasketch_theta_estimate(datasketch_theta_intersect(12, a.s, b.s)) AS retained,    -- in both
...
`K = 200` is a sensible production starting point. Higher `K` → smaller error, larger sketch. Use [`datasketch_kll_normalized_rank_error`](#datasketch_kll_normalized_rank_error) to inspect the actual rank-error guarantee for the configured `K`.
...
The result is a list of structs — `(item, estimate, lower_bound, upper_bound)`. Use `'NO_FALSE_POSITIVES'` for items that are *definitely* heavy hitters, `'NO_FALSE_NEGATIVES'` for the union of all candidates that *might* be. The leading `1024` is `map_size` (a power of two) — bigger sketches keep more candidates and tighten the per-item error bound.
...
```sql
SELECT
  datasketch_req_quantile(datasketch_req(12, value), 0.001) AS p001,
  datasketch_req_quantile(datasketch_req(12, value), 0.5)   AS p50,
  datasketch_req_quantile(datasketch_req(12, value), 0.999) AS p999
FROM measurements;
```
