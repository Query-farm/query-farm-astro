  ...
  if (section && section.slug !== CONCEPTS.slug) {
    ...
    for (const tag of route.head) {
      if (tag.tag === 'title' && typeof tag.content === 'string') {
        ...
        // Starlight's default og:title is the bare frontmatter title with no
        // site-name suffix at all — unlike <title>, there's no existing
        // "VGI Documentation" to find-and-replace — so append the same
        // per-section suffix directly to keep the social-preview title from
        // being identical across every language's copy of a shared page.
        tag.attrs.content = `${tag.attrs.content} | ${siteTitle}`;
      }
    }
  }

  // Starlight's own head builder emits no og:image/twitter:image at all (see
  // head.ts upstream), so every one of these pages had no social-preview
  // image whatsoever. One card per section — not per page — generated by
  // scripts/generate-vgi-docs-og-images.mjs (npm run generate:og-vgi-docs);
  // the landing page and bare Concepts pages get the Concepts card.
  const ogImageSlug = section && section.slug !== CONCEPTS.slug ? section.slug : CONCEPTS.slug;
  const ogImageUrl = `${SITE_ORIGIN}/og/vgi-docs/${ogImageSlug}.png`;
  route.head.push(
